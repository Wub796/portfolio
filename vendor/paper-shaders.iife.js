...
}`;function Re(o){return"paperShaderMount"in o}function Wo(){let o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function Yo(o){let e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,r=window.innerWidth-o.documentElement.clientWidth,c=e*a+r,n=outerWidth/c,u=Math.round(100*n);return u%5===0?u/100:u===33?1/3:u===67?2/3:u===133?4/3:n}var Me={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Oe={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},ze={none:0,contain:1,cover:2};var d=`
...
`;var Ue={maxSamples:50},vo=`#version 300 es
precision mediump float;
...
`;function _o(o){let e=document.createElement("canvas"),a=1e3;return new Promise((r,c)=>{let n=new Image;n.crossOrigin="anonymous",n.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(n.width=a,n.height=a);let u=n.naturalWidth/n.naturalHeight,t=Math.floor(a*.15),p=Math.ceil(t*2.5),s=a,g=a;u>1?g=Math.floor(a/u):s=Math.floor(a*u),e.width=s+2*p,e.height=g+2*p;let f=e.getContext("2d",{willReadFrequently:!0});if(!f)throw new Error("Failed to get canvas 2d context");f.fillStyle="white",f.fillRect(0,0,e.width,e.height),f.drawImage(n,p,p,s,g);let{width:m,height:I}=e,v=f.getImageData(0,0,m,I).data,i=m*I,l=new Uint8ClampedArray(i);for(let M=0;M<i;M++){let E=M*4,D=v[E]??0,X=v[E+1]??0,L=v[E+2]??0;l[M]=.299*D+.587*X+.114*L|0}let A=t,w=Math.max(1,Math.round(.12*t)),V=5,F=Ve(l,m,I,A,3),C=Ve(l,m,I,w,3),x=Ve(l,m,I,V,1),R=f.createImageData(m,I),Y=R.data;for(let M=0;M<i;M++){let E=M*4;Y[E]=x[M]??0,Y[E+1]=F[M]??0,Y[E+2]=C[M]??0,Y[E+3]=255}f.putImageData(R,0,0),e.toBlob(M=>{if(!M){c(new Error("Failed to create PNG blob"));return}r({blob:M})},"image/png")}),n.addEventListener("error",()=>{c(new Error("Failed to load image"))}),n.src=typeof o=="string"?o:URL.createObjectURL(o)})}function ho(o,e,a,r){if(r<=0)return o.slice();let c=new Uint8ClampedArray(e*a),n=new Uint32Array(e*a);for(let u=0;u<a;u++){let t=0;for(let p=0;p<e;p++){let s=u*e+p,g=o[s]??0;t+=g,n[s]=t+(u>0?n[s-e]??0:0)}}for(let u=0;u<a;u++){let t=Math.max(0,u-r),p=Math.min(a-1,u+r);for(let s=0;s<e;s++){let g=Math.max(0,s-r),f=Math.min(e-1,s+r),m=p*e+f,I=p*e+(g-1),P=(t-1)*e+f,v=(t-1)*e+(g-1),i=n[m]??0,l=g>0?n[I]??0:0,A=t>0?n[P]??0:0,w=g>0&&t>0?n[v]??0:0,V=i-l-A+w,F=(f-g+1)*(p-t+1);c[u*e+s]=Math.round(V/F)}}return c}function Ve(o,e,a,r,c){if(r<=0||c<=1)return ho(o,e,a,r);let n=o,u=o;for(let t=0;t<c;t++)u=ho(n,e,a,r),n=u;return u}var yo=`#version 300 es
precision mediump float;
...
`,q={measurePerformance:!1,workingSize:512,iterations:40};function Ao(o){let e=document.createElement("canvas"),a=e.getContext("2d"),r=typeof o=="string"&&o.startsWith("blob:");return new Promise((c,n)=>{if(!o||!a){n(new Error("Invalid file or canvas context"));return}let u=r&&fetch(o).then(s=>s.headers.get("Content-Type")),t=new Image;t.crossOrigin="anonymous";let p=performance.now();t.onload=async()=>{let s,g=await u;g?s=g==="image/svg+xml":typeof o=="string"?s=o.endsWith(".svg")||o.startsWith("data:image/svg+xml"):s=o.type==="image/svg+xml";let f=t.width||t.naturalWidth,m=t.height||t.naturalHeight;if(s){let _=f/m;f>m?(f=4096,m=4096/_):(m=4096,f=4096*_),t.width=f,t.height=m}let I=Math.min(f,m),v=q.workingSize/I,i=Math.round(f*v),l=Math.round(m*v);q.measurePerformance&&(console.log("[Processing Mode]"),console.log(`  Original: ${f}\xD7${m}`),console.log(`  Working: ${i}\xD7${l} (${(v*100).toFixed(1)}% scale)`),v<1&&console.log(`  Speedup: ~${Math.round(1/(v*v))}\xD7`)),e.width=f,e.height=m;let A=document.createElement("canvas");A.width=i,A.height=l;let w=A.getContext("2d");w.drawImage(t,0,0,i,l);let V=performance.now(),C=w.getImageData(0,0,i,l).data,x=new Uint8Array(i*l),R=new Uint8Array(i*l),Y=0;for(let h=0,_=0;h<C.length;h+=4,_++){let O=C[h+3]===0?0:1;x[_]=O,Y+=O}let M=[],E=[];for(let h=0;h<l;h++)for(let _=0;_<i;_++){let S=h*i+_;if(!x[S])continue;let O=!1;_===0||_===i-1||h===0||h===l-1?O=!0:O=!x[S-1]||!x[S+1]||!x[S-i]||!x[S+i]||!x[S-i-1]||!x[S-i+1]||!x[S+i-1]||!x[S+i+1],O?(R[S]=1,M.push(S)):E.push(S)}q.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-V).toFixed(2)}ms`),console.log(`  Shape pixels: ${Y} / ${i*l} (${(Y/(i*l)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${E.length}`),console.log(`  Boundary pixels: ${M.length}`));let D=Lo(x,R,new Uint32Array(E),new Uint32Array(M),i,l),X=performance.now(),L=Ho(D,x,R,i,l);q.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-X).toFixed(2)}ms`);let j=0,H;for(let h=0;h<E.length;h++){let _=E[h];L[_]>j&&(j=L[_])}let $=document.createElement("canvas");$.width=i,$.height=l;let le=$.getContext("2d"),Q=le.createImageData(i,l);for(let h=0;h<l;h++)for(let _=0;_<i;_++){let S=h*i+_,O=S*4;if(!x[S])Q.data[O]=255,Q.data[O+1]=255,Q.data[O+2]=255,Q.data[O+3]=0;else{let oe=255*(1-L[S]/j);Q.data[O]=oe,Q.data[O+1]=oe,Q.data[O+2]=oe,Q.data[O+3]=255}}le.putImageData(Q,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage($,0,0,i,l,0,0,f,m);let T=a.getImageData(0,0,f,m),ee=document.createElement("canvas");ee.width=f,ee.height=m;let J=ee.getContext("2d");J.drawImage(t,0,0,f,m);let ce=J.getImageData(0,0,f,m);for(let h=0;h<T.data.length;h+=4){let _=ce.data[h+3],S=T.data[h+3];_===0?(T.data[h]=255,T.data[h+1]=0):(T.data[h]=S===0?0:T.data[h],T.data[h+1]=_),T.data[h+2]=255,T.data[h+3]=255}a.putImageData(T,0,0),H=T,e.toBlob(h=>{if(!h){n(new Error("Failed to create PNG blob"));return}if(q.measurePerformance){let _=performance.now()-p;if(console.log(`[Total Processing Time] ${_.toFixed(2)}ms`),v<1){let S=_*Math.pow(f*m/(i*l),1.5);console.log(`[Estimated time at full resolution] ~${S.toFixed(0)}ms`),console.log(`[Time saved] ~${(S-_).toFixed(0)}ms (${Math.round(S/_)}\xD7 faster)`)}}c({imageData:H,pngBlob:h})},"image/png")},t.onerror=()=>n(new Error("Failed to load image")),t.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Lo(o,e,a,r,c,n){let u=a.length,t=new Int32Array(u*4);for(let p=0;p<u;p++){let s=a[p],g=s%c,f=Math.floor(s/c);t[p*4+0]=g<c-1&&o[s+1]?s+1:-1,t[p*4+1]=g>0&&o[s-1]?s-1:-1,t[p*4+2]=f>0&&o[s-c]?s-c:-1,t[p*4+3]=f<n-1&&o[s+c]?s+c:-1}return{interiorPixels:a,boundaryPixels:r,pixelCount:u,neighborIndices:t}}function Ho(o,e,a,r,c){let n=q.iterations,u=.01,t=new Float32Array(r*c),{interiorPixels:p,neighborIndices:s,pixelCount:g}=o,f=performance.now(),m=1.9,I=[],P=[];for(let v=0;v<g;v++){let i=p[v],l=i%r,A=Math.floor(i/r);(l+A)%2===0?I.push(v):P.push(v)}for(let v=0;v<n;v++){for(let i of I){let l=p[i],A=s[i*4+0],w=s[i*4+1],V=s[i*4+2],F=s[i*4+3],C=0;A>=0&&(C+=t[A]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),F>=0&&(C+=t[F]);let x=(u+C)/4;t[l]=m*x+(1-m)*t[l]}for(let i of P){let l=p[i],A=s[i*4+0],w=s[i*4+1],V=s[i*4+2],F=s[i*4+3],C=0;A>=0&&(C+=t[A]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),F>=0&&(C+=t[F]);let x=(u+C)/4;t[l]=m*x+(1-m)*t[l]}}if(q.measurePerformance){let v=performance.now()-f;console.log(`[Optimized Poisson Solver (SOR \u03C9=${m})]`),console.log(`  Working size: ${r}\xD7${c}`),console.log(`  Iterations: ${n}`),console.log(`  Time: ${v.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${g}`),console.log(`  Speed: ${(n*g/(v*1e3)).toFixed(2)} Mpixels/sec`)}return t}var bo={none:0,circle:1,daisy:2,diamond:3,metaballs:4};var Co=`#version 300 es
precision mediump float;
